...
import { MapPin, Bed, Bath, Ruler, Phone, Mail, MessageCircle, Calendar, Zap, Car, ArrowUpFromDot, Play } from 'lucide-react';
...
const PublicListingView = ({ listing, sign, translation, lang, onLangChange }: PublicListingViewProps) => {
      ...
      <header className="border-b border-border bg-card sticky top-0 z-40">
        <div className="container-wide flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-3">
            ...
            <LanguageSwitcher current={lang} onChange={onLangChange} compact />
          </div>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
          <div className="lg:col-span-2 space-y-6">
            <div>
              ...
              <h1 className="font-display text-2xl sm:text-3xl font-bold text-foreground">{displayTitle}</h1>
              {fullLocation && (
            ...
            {displayDescription && (
              <>
                ...
                <div>
                  ...
                  <div className="text-muted-foreground whitespace-pre-line leading-relaxed">{displayDescription}</div>
                </div>
              </>
            )}
            ...
            <div className="bg-card rounded-2xl border border-border p-6 sticky top-20">
              {listing.contact_name && (
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    {listing.agency_name && <p className="text-xs text-muted-foreground">{listing.agency_name}</p>}
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2 mb-6">
                {listing.show_phone && listing.contact_phone && (
                  <a href={`tel:${listing.contact_phone}`} className="flex items-center gap-2 w-full px-4 py-2.5 rounded-xl bg-primary text-primary-foreground font-medium text-sm hover:bg-primary-dark transition-colors">
                    <Phone className="w-4 h-4" /> {listing.contact_phone}
                  </a>
                )}
                {listing.show_email && listing.contact_email && (
                  <a href={`mailto:${listing.contact_email}`} className="flex items-center gap-2 w-full px-4 py-2.5 rounded-xl border border-border text-foreground font-medium text-sm hover:bg-muted transition-colors">
                    <Mail className="w-4 h-4" /> {listing.contact_email}
                  </a>
                )}
                {listing.show_whatsapp && listing.contact_whatsapp && (
                  <a href={`https://wa.me/${listing.contact_whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 w-full px-4 py-2.5 rounded-xl bg-success text-success-foreground font-medium text-sm hover:opacity-90 transition-opacity">
